...
BATCH_SIZE = 1518
CONTINUE_FROM_MODEL=0

EPOCHS = 160
PATIENCE = 30

parser = argparse.ArgumentParser()
parser.add_argument('-n', '--N_MODELS', default=N_MODELS, type=int)
...
parser.add_argument('-c', '--CONTINUE_FROM_MODEL', default=CONTINUE_FROM_MODEL, type=int)
...
def compute_mean_topology_count(activations, labels):
    ...
    for cls in torch.unique(labels):
        activations_class = activations[labels==cls,...]
...
def compute_topology_count_for_model(model, train_loader):
    ...
    return compute_mean_topology_count(activations, targets)
...
if __name__ == "__main__":
   for model_idx in range(CONTINUE_FROM_MODEL, CONTINUE_FROM_MODEL+N_MODELS):
        ...
        with open("{}/val_stopped/model_{}/model_{}_samples.json".format(model_training_folder, model_idx, model_idx), 'w') as sample_f:
            samples = {"train": list(samples_train), "val": list(samples_val), "test": list(samples_test)}
            json.dump(samples, sample_f, default=int)
        ...
        val_loader = torch.utils.data.DataLoader(valset, batch_size=BATCH_SIZE, num_workers=0)
        ...
        for epoch in range(EPOCHS):
                ...
                avg_vloss, vacc = evaluate_model(model, val_loader)
                ...
                if epoch % SAVE_EVERY == 0:
                    ...
                    torch.save(model, model_path)
                model_losses[epoch] = (avg_loss, avg_vloss.item(), vacc.item())
        ...
        with open("{}/topology_stopped/model_{}/model_{}_samples.json".format(model_training_folder, model_idx, model_idx), 'w') as sample_f:
            samples = {"train": list(samples_train), "val": list(samples_val)}
            json.dump(samples, sample_f, default=int)
        ...
        for epoch in range(EPOCHS):
                ...
                top_count = compute_topology_count_for_model(model, train_loader)
                ...
                if epoch % SAVE_EVERY == 0:
                    ...
                    torch.save(model, model_path)
                model_topology[epoch] = (top_count)
                with open(f'{model_training_folder}/topology_stopped/model_{model_idx}/model_{model_idx}_topology_data.json', 'w') as f:
                    json.dump(model_topology, f)
